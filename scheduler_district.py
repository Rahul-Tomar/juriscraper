...
def setup_logging():
    ...
    log_filename = f"district_{today}.log"
...
def process_opinion_data(opinion, opinion_date, year, court_name, court_type, class_name, state_name):
    ...
    data = {
        # required
        'title': check_none(opinion.get('case_names')), 'pdf_url': check_none(opinion.get('download_urls')), 'date': datetime.strptime(opinion_date, "%d/%m/%Y"), 'case_status': check_none(opinion.get('precedential_statuses')), 'docket': docket,
        # optional
        ...
        # extra
        'blocked_statuses': check_none(opinion.get('blocked_statuses')), 'case_name_shorts': check_none(opinion.get('case_name_shorts')), 'opinion_type': check_none(opinion.get('opinion_types')), 'html_url': check_none(opinion.get('html_urls')), 'response_html': check_none(opinion.get('response_htmls')),
        # additional
...
def main():
    ...
    logger.info(f"Scheduler set up. Running jobs at {SCHEDULING_HOUR}:{SCHEDULING_MINUTE} daily.")
